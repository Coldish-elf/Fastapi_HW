...
def create_task(task: TaskCreate, db: Session = Depends(get_db), username: str = Depends(get_current_user)):
    owner = db.query(User).filter(User.username == username).first() if username else None
    db_task = Task(title=task.title, description=task.description, status=task.status, priority=task.priority, owner=owner)
    db.add(db_task)
    db.commit()
    db.refresh(db_task)
...
def read_tasks(
    ...
    if search:
        search_str = f"%{search}%"
        query = query.filter((Task.title.like(search_str)) | (Task.description.like(search_str)))
...
def update_task(
    task_id: int, update_data: TaskCreate, db: Session = Depends(get_db), username: str = Depends(get_current_user)
):
