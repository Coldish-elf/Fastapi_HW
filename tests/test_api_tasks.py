...
def create_task_direct(
    ...
    task = Task(
        title=title, owner_id=owner_id, priority=priority, status=status, description=""
    )
    db.add(task)
    db.commit()
    db.refresh(task)
...
def test_create_task(
    client: TestClient, auth_headers: dict, db_session: Session, test_user
):
...
def test_read_tasks_sorting(
    ...
    client.post(
        ...
        json={"title": "C Task", "priority": 1, "status": "pending"},
    )
    client.post(
        "/tasks",
        headers=auth_headers,
        json={"title": "A Task", "priority": 3, "status": "done"},
    )
    client.post(
        "/tasks",
        headers=auth_headers,
        json={"title": "B Task", "priority": 2, "status": "in_progress"},
    )
    ...
    assert [task["status"] for task in tasks_status] == [
        "done",
        "in_progress",
        "pending",
    ]
...
def test_read_tasks_search(
    ...
    client.post(
        "/tasks",
        headers=auth_headers,
        json={"title": "Searchable Alpha", "description": "Unique keyword"},
    )
    client.post(
        "/tasks",
        headers=auth_headers,
        json={"title": "Another Task", "description": "Common text"},
    )
    client.post(
        "/tasks",
        headers=auth_headers,
        json={"title": "Specific Beta", "description": "Contains Alpha word"},
    )
...
def test_read_tasks_top_n(
    ...
    client.post(
        "/tasks", headers=auth_headers, json={"title": "P5_another", "priority": 5}
    )
...
def test_update_task_forbidden(
    client: TestClient, db_session: Session, test_user, auth_headers
):
    ...
    other_task_response = client.post(
        ...
        json={"title": "Other's Task", "priority": 1},
    )
    ...
    response = client.put(
        f"/tasks/{other_task_id}", headers=auth_headers, json=update_data
    )
...
def test_delete_task_forbidden(
    ...
    other_task_response = client.post(
        "/tasks",
        headers=other_auth_headers,
        json={"title": "Other's Task To Delete", "priority": 1},
    )
    other_task_id = other_task_response.json()["id"]
